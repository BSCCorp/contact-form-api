...
describe("contact forms API", () => {
  ...
  it("gets a contact form", async () => {
    const result = await getContactForm("contact-1");

    expect(result.data.name).toBe("John Smith");
    expect(result.data.email).toBe("john@example.com");
    expect(result.data.subject).toBe("I need help");
  });
  ...
  it("creates a contact form", async () => {
    ...
    expect(result.data._id).toBe("contact-new");
  });

  it("deletes a contact form", async () => {
    await expect(
      deleteContactForm("contact-1")
    ).resolves.toBeNull();
  });
